...
.main-list .ml-item {
    ...
    line-height: calc(7*var(--aspect-ratio));
    padding: 5% .5%;
    border: 1px solid rgba(0, 116, 141, 0.15);
...
<div class="main-section">
        <div class="olist-container">
            ...
            <div class="offer-table" style="width: 98% !important; overflow-x:auto;">
                <table class="oli-item" width="100%">
                    <thead>
                        <tr class="head-list">
                            <th class="hl-item1"><?php echo lang('offer_name') ?></th>
    ...
    <style>
        .oli-item {
          ...
          overflow-x:hidden;
            ...
            width: 100%;
            overflow: auto;
             /*  Allow setting height and overflow
...
$(document).ready(function() {
   ...
   let scanner = null;
    $(document).on('click', '#start-scan',function() {
        console.log('scan_process_start');
        $('#status').text('');
           ...
           //alert(qr_code);
            $.ajax({
				...
				beforeSend: function() {
                    // stopCamera();
                    $('#qr-video').hide();
                    $(".scanner-imgs").html('<?php echo lang('please_wait') ?> ...');
				// 	$('#body-preloader').css('display', 'block');
				},
