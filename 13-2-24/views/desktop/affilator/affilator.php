            ...
            <div class="col-lg-3 px-4 my-2 text-center">
              ...
              <?php $currency_code = $this->input->cookie('currency_code',true); ?>
                ...
                if(!empty($totalCommission)){
                  // echo getCurrencySymbol($currency_code).get_price_without_symbol($totalCommission,$default_currency);// = (number_format($totalCommission->amount, 2));
                  echo get_currency($totalCommission,$default_currency); 
                  }else{
                      echo 0;   
                  }
                ?>
              ...
              <table class="table table-bordered text-center rounded-2">
                <thead>
                  <tr class="f-Poppins">
                    ...
                    <th class="text-sky fw-600 py-3 fs-20 table-text-media"> <?php echo lang('commission_percentage')?> </th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="bg-white text-sky fw-bold fs-20 table-text-media"><?= get_currency($list->amount,$list->currency_type); ?></td>
...
$('#nefdevice_register_btn').on('click',function(){
  $('#status').text('');
            ...
            if (serialNumber === "") {
                $('#status').text("<?= lang('input_is_blank_or_contains_only_whitespace'); ?>");
                return false;
            }else{
                var serialNumber = serialNumber; 
                $.ajax({
