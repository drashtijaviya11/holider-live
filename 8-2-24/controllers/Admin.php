            ...
            if(!empty($unrdmPrice)){
                foreach($unrdmPrice as $unrdmPrice_val){
                    if(!empty($unrdmPrice_val['price']) && $unrdmPrice_val['price']!=0){
                        $new_unrdm = get_currency_without_symbol($unrdmPrice_val['price'],$list['default_currency']);
                    }else{
                        $new_unrdm = 0;
                    }
            ...
            if(!empty($remPrice)){
                foreach($remPrice as $remPrice_val){
                    if(!empty($remPrice_val['price']) && $remPrice_val['price'] != 0){
                        $new_rdm = get_currency_without_symbol($remPrice_val['price'],$list['default_currency']);
                    }else{
                        $new_rdm = 0;
                    }
                    $rdm= $rdm + $new_rdm;
        ...
        if(!empty($unpaid_trasnation)){
            foreach($unpaid_trasnation as $unpaid_trasnation_val){
                if(!empty($unpaid_trasnation_val['amount']) && $unpaid_trasnation_val['amount'] != 0){
                    $new_unpaid_amount = get_currency_without_symbol($unpaid_trasnation_val['amount'],$list['default_currency']);
                }else{
                    $new_unpaid_amount = 0;
                }
                if(!empty($unpaid_trasnation_val['comission']) && $unpaid_trasnation_val['comission'] != 0){
                    $new_comission = get_currency_without_symbol($unpaid_trasnation_val['comission'],$list['default_currency']);
                }else{
                    $new_comission = 0;
                }
                if(!empty($unpaid_trasnation_val['affialiator_commission']) && $unpaid_trasnation_val['affialiator_commission'] != 0){
                    $new_aff_comission = get_currency_without_symbol($unpaid_trasnation_val['affialiator_commission'],$list['default_currency']);
                }else{
                    $new_aff_comission = 0;
                }
        ...
        if(!empty($paid_trasnation)){
            foreach($paid_trasnation as $paid_trasnation_val){
                if(!empty($paid_trasnation_val['amount']) && $paid_trasnation_val['amount'] != 0){
                    $new_paid_amount = get_currency_without_symbol($paid_trasnation_val['amount'],$list['default_currency']);
                }else{
                    $new_paid_amount = 0;
                }
        ...
        if(!empty($credit_amount)){
            foreach($credit_amount as $credit_amount_val){
                if(!empty($credit_amount_val['amount'])){
                    $new_amt = get_currency_without_symbol($credit_amount_val['amount'],$list['default_currency']);
                }else{
                    $new_amt = 0;
                }
        ...
        if(!empty($debit_amount)){
            foreach($debit_amount as $debit_amount_val){
                if(!empty($debit_amount_val['amount'])){
                    $new_dbt = get_currency_without_symbol($debit_amount_val['amount'],$list['default_currency']);
                }else{
                    $new_dbt = 0;
                }
            ...
            if(!empty($totalamount)){
                ...
                foreach($totalamount as $totalamount_val){
                    ...
                    if(!empty($totalamount_val['total_amount'])){
                        ...
                        $new_totalamount = get_currency_without_symbol($totalamount_val['total_amount'],$offer['currency_type']);
     ...
     public function providerOfferList()
     {
        ...
        foreach($providerOffer as &$offer){
            ...
            if(!empty($unpaid_trasnation)){
                foreach($unpaid_trasnation as $unpaid_trasnation_val){
                    if(!empty($unpaid_trasnation_val['amount']) && $unpaid_trasnation_val['amount'] != 0){
                        $new_unpaid_amount = get_currency_without_symbol($unpaid_trasnation_val['amount'],$offer['currency_type']);
                    }else{
                        $new_unpaid_amount = 0;
                    }
                    if(!empty($unpaid_trasnation_val['comission']) && $unpaid_trasnation_val['comission'] != 0){
                        $new_comission = get_currency_without_symbol($unpaid_trasnation_val['comission'],$offer['currency_type']);
                    }else{
                        $new_comission = 0;
                    }
                    if(!empty($unpaid_trasnation_val['affialiator_commission']) && $unpaid_trasnation_val['affialiator_commission'] != 0){
                        $new_aff_comission = get_currency_without_symbol($unpaid_trasnation_val['affialiator_commission'],$offer['currency_type']);
                    }else{
                        $new_aff_comission = 0;
                    }
                    // $unpaid_total = $unpaid_total + $new_unpaid_amount;
