    ...
    { 
		...
		if(!empty($creditedData)){
			...
			foreach ($creditedData as $creditedDataval){
				...
				$new_amount = get_price_without_symbol($creditedDataval['amount'],$creditedDataval['currency_type'],$creditedDataval['created_at']);
                }else{
                    $new_amount= 0;
                }
		...
		if(!empty($debitedData)){
			...
			foreach ($debitedData as $debitedDataval){
				...
				$new_debit_amount = get_price_without_symbol($debitedDataval['amount'],$debitedDataval['currency_type'],$debitedDataval['created_at']);
                }else{
                    $new_debit_amount= 0;
                }
