...
@media (max-width: 767px) {
        .temp,label {
            font-size: 10px;
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-12 text-center whatsapp">Add Affiliator</div>
                        <hr>
                    </div>
                    <form action="" id="add_user"  method="post" >
                        <div class="row mt-2">
                            <div class="col-6">
                                ...
                                <input type="text" name="name" class="form-control temp rqure" placeholder="<?= lang('enter_your_name') ?>">
                            </div>
                            <div class="col-6">
                                <label for=""><?= lang('email') ?></label><i class="bi bi-star-fill"></i>
                                <input type="text" name="email" class="form-control temp rqure" placeholder="<?= lang('enter_email') ?>">
                            </div>
                        </div>
                        <!-- <div class="row mt-2">
                        ...
                        <div class="row mt-2">
                            <div class="col-6">
                                ...
                                <select class="form-select form-control temp" name="status" id="category">
                            ...
                            <div class="col-6">
                                ...
                                <input type="text" name="phone" class="form-control temp rqure" placeholder="<?= lang('enter_mobile_no') ?>"  style="margin:0px">
    ...
    $(document).ready(function(){
        $("#add_user").submit(function(e){
			...
			$(this).find('.rqure, select').each(function () {
                ...
                if ($(this).is('.rqure')) {
					// For text inputs, check if the value is empty or contains only whitespace
					if ($(this).val().trim() === "") {
						formValid = false;
						return false; // exit the loop
					}
...
/**********Loader**************/ 
            ...
            $.ajax({
                url : "<?= base_url('admin/save_affiliator') ?>",
                data : formData,
                dataType : "json",
                type : "POST",
                contentType: false,
                ...
                success : function(res)
                {
                    ...
                    if(res.status == true)
                    {
                        ...
                        $(".tbodyAffil").append('<tr id='+res.id+'>'+res.html+'</tr>');
                    }
					else{
                        new Noty({
                            text: res.Message,
