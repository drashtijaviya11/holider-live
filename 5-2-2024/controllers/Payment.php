...
// 			return $customer->id;
				...
				if($charge){
					...
					if($charge['amount_refunded'] == 0 && empty($charge['failure_code']) && $charge['paid'] == 1 && $charge['captured'] == 1){
                        ...
                        $insert_data = array(
                            ...
                            'currency_type' => strtoupper($currencytype),
                            'pickup_date' => $picDate,
                            'pickup_time' => $picTime,
    ...
    public function  success_payment(){
        ...
        $data->currency_type = strtoupper($data->currency_type);
		...
		$redirect_url = 'payment/payment_success_status?' . http_build_query(['payment_status' => $order, 'inserted_value' => json_encode($datanew),'trans_id'=>$key,'offer_id'=>$data->offer_id,'total_amount'=>$data->total_amount,'offer_name'=>$offers['name'],'adult_qty'=>$data->qty,'child_qty'=>$data->child_qty]);
    ...
    public function purchaseOffer(){
        ...
        //$this->session->buy_now_session('logged_in')
...
$pickup_date =$picDate;
	// if(!empty($time)){
	// $createdDate = $picDate;//new DateTime($offers['created_at']);
    //     $dateTime = DateTime::createFromFormat('d/m/Y', $createdDate);
	// $time_converting = DateTime::createFromFormat('H:i', $time);
    //     $hour = $time_converting->format('H');
    //     $minute = $time_converting->format('i');
    //     $dateTime->setTime($hour, $minute, 0);
    //     // Format the date in the desired format
    //     $final_pickup = $dateTime->format('Y-m-d H:i:s');
	// $new_expire = new DateTime($final_pickup);
    //     }else{
	...
	// }
        ...
        $insert_data = array(
            ...
            'transaction_key' =>$key,
            'purchase_date' => date('Y-m-d H:i:s'),//$formattedDate,//$offers['created_at'],
            'pickup_date' => $pickup_date,
            'pickup_time' => $pickup_time,
            //date('Y-m-d H:i:s'),//$offers['created_at'],
    ...
    if($offet_type=='both'){
        for($i=0; $i<$child_Qnty; $i++)
        {
            ...
            // $this->data['order_items'] = 'aaaa';
                ...
                send_mail_new($provider_details['email'], 'Holider', $this->load->view('email_template_provider', $this->data, true),$attach);
            }
        }
