...
class Provider extends CI_Controller
{
    ...
    { 
		...
		if(!empty($creditedData)){
			...
			foreach ($creditedData as $creditedDataval){
				...
				$new_amount = get_price_without_symbol($creditedDataval['amount'],$creditedDataval['currency_type']);
                }else{
                    $new_amount= 0;
                }
		...
		if(!empty($debitedData)){
			...
			foreach ($debitedData as $debitedDataval){
				...
				$new_debit_amount = get_price_without_symbol($debitedDataval['amount'],$debitedDataval['currency_type']);
                }else{
                    $new_debit_amount= 0;
                }
	...
	public function save_employee() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $response['status'] = false;
            $response['message'] = translate_static_function('eng',$lang,validation_errors());;
            echo json_encode($response);
            return;
        } else {
